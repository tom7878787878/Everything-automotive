...
// --- Page Navigation ---
// Added a parameter for optional data to be passed to page-specific load functions
...
onAuthStateChanged(auth, user => {
    ...
    if (user) {
        ...
        // If coming from auth page, automatically switch to garage (or home/products)
        if (document.querySelector('.page.active')?.id === 'auth') {
            showPage('home'); // or 'garage' depending on desired post-login page
        }
        clearAuthFields(); 
        ...
        // Note: The `renderSavedVehicles()` and `loadWishlist()` will handle displaying empty states
        clearAuthFields(); 
...
document.getElementById("loginForm").addEventListener("submit", async (e) => {
    ...
    try {
        ...
        } else if (err.code === 'auth/invalid-credential') { // More generic for failed login
            errorMessage = "Invalid email or password.";
            displayFormError('loginEmailError', errorMessage);
            displayFormError('loginPassError', ' '); // Clear password error if combo failed
        }
        else {
...
const wishlistEmptyMessage = document.querySelector('#wishlist .no-items-message'); // Specific message for wishlist
...
featuredProductsGrid.addEventListener('click', (event) => {
    if (event.target.classList.contains('add-to-wishlist-btn')) {
        ...
        const product = {
            ...
            price: parseFloat(productCard.dataset.productPrice), // Ensure price is a number
            amazonUrl: productCard.dataset.amazonUrl,
            imageUrl: productCard.dataset.imageUrl,
            // Extract brand/etc. if needed for display, or save from original card
...
wishlistItemsContainer.addEventListener('click', (event) => {
    if (event.target.classList.contains('remove-from-wishlist-btn')) {
        ...
        removeFromWishlist(productIdToRemove, event.target); // Pass button for loading state
    }
});
...
clearWishlistButton.addEventListener('click', async () => {
    if (confirm("Are you sure you want to clear your entire wishlist?")) {
        await clearWishlist(clearWishlistButton); // Pass button for loading state
    }
});

// Helper to get user's garage document reference
// (Already defined for garage, but clarifying its use for wishlist)
// function getUserGarageDocRef() { ... } 

async function getWishlistFromFirestore() {
    const userDocRef = getUserGarageDocRef();
    if (!userDocRef) return []; // Return empty if not logged in
...
async function addToWishlist(product) {
    ...
    try {
        ...
        // Check if product already exists by ID to prevent duplicates
...
async function removeFromWishlist(productId, button) {
    ...
    try {
        const currentWishlist = await getWishlistFromFirestore();
        // Find the exact item to remove by its ID
...
document.addEventListener('DOMContentLoaded', () => {
    showPage('home'); // Ensure a page is active on initial load
    // These will now be called by onAuthStateChanged if a user is logged in
    // Otherwise, the functions themselves handle the "please log in" message
    // loadVehicle(); // Removed: renderSavedVehicles handles initial load for garage
    // loadWishlist(); // Already called by onAuthStateChanged
});
